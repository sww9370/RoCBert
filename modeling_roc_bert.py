...
class RoCBertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.merge_input_way = config.merge_input_way

        if self.merge_input_way == "concat":
    ...
    def forward(
        ...
        if self.merge_input_way == "plus":
            ...
            return embedding_in, None, None
        elif self.merge_input_way == "concat":
        ...
        elif self.merge_input_way == "attention":
            embeddings = inputs_embeds + token_type_embeddings
            if self.position_embedding_type == "absolute":
                position_embeddings = self.position_embeddings(position_ids)
                embeddings += position_embeddings
            ...
            if self.enable_pronunciation:
                ...
                embedding_pronunciation = None
            return embeddings, embedding_shape, embedding_pronunciation
        else:
            raise ValueError("Invalid merge_input_way, require value: ['plus', 'concat', 'attention']")
...
class RoCBertAdditionalAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        if config.hidden_size % config.num_attention_heads != 0 and not hasattr(config, "embedding_size"):
            raise ValueError(
        ...
        shape_embed_size = config.shape_embed_dim
        self.key_shape = nn.Linear(shape_embed_size, self.all_head_size)
        self.value_shape = nn.Linear(shape_embed_size, self.all_head_size)
        logger.info(f"add shape attention, shape_embed_size={shape_embed_size}")
        pronunciation_embed_size = config.pronunciation_embed_dim
        self.key_pronunciation = nn.Linear(pronunciation_embed_size, self.all_head_size)
        self.value_pronunciation = nn.Linear(pronunciation_embed_size, self.all_head_size)
        logger.info(f"add pronunciation attention, pronunciation_embed_size={pronunciation_embed_size}")

        self.output = RoCBertSelfOutput(config)

    def get_attention_scores(self, query_layer, key_layer, attention_mask):
        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = torch.sum(torch.mul(query_layer, key_layer), dim=2)
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)
    ...
    def forward(
        ...
        past_key_value: Optional[Tuple[Tuple[torch.FloatTensor]]] = None,
        output_attentions: Optional[bool] = False,
        shape_embed: Optional[torch.FloatTensor] = None,  # batch_size * seq_len * hidden_size
        pronunciation_embed: Optional[torch.FloatTensor] = None,
    ):
        query_layer = self.query(hidden_states)  # batch_size * seq_len * hidden_size
        key_layer_h = self.key(hidden_states)
        key_layer_s = self.key_shape(shape_embed)
        key_layer_p = self.key_pronunciation(pronunciation_embed)
        value_layer_h = self.value(hidden_states)
        value_layer_s = self.value_shape(shape_embed)
        value_layer_p = self.value_pronunciation(pronunciation_embed)

        attention_scores_h = self.get_attention_scores(query_layer, key_layer_h, attention_mask)
        attention_scores_s = self.get_attention_scores(query_layer, key_layer_s, attention_mask)
        attention_scores_p = self.get_attention_scores(
            query_layer, key_layer_p, attention_mask
        )  # batch_size * seq_len

        attention_scores = torch.stack(
            (attention_scores_h, attention_scores_s, attention_scores_p), dim=-1
        )  # batch_size *seq_len *  3
        attention_scores = nn.Softmax(dim=-1)(attention_scores)  # normalize

        attention_scores_h, attention_scores_s, attention_scores_p = torch.split(
            attention_scores, 1, dim=-1
        )  # batch_size * seq_len *  1
        mix_layer = (
            torch.mul(attention_scores_h, value_layer_h)
            + torch.mul(attention_scores_s, value_layer_s)
            + torch.mul(attention_scores_p, value_layer_p)
        )

        attention_output = self.output(mix_layer, hidden_states)
...
class RoCBertEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        if hasattr(config, "additional_attention_layer") and config.merge_input_way == "attention":
            if config.additional_attention_layer < 0 or config.additional_attention_layer > config.num_hidden_layers:
                raise ValueError(f"additional_attention_layer should be in range [0, {config.num_hidden_layers}]")
            layer_list = list()
            for _ in range(config.additional_attention_layer):
                layer_list.append(RoCBertLayer(config))
            layer_list.append(RoCBertAdditionalAttention(config))
            for _ in range(config.additional_attention_layer, config.num_hidden_layers):
                layer_list.append(RoCBertLayer(config))
            self.layer = nn.ModuleList(layer_list)
            self.additional_attention_layer = config.additional_attention_layer
        else:
            self.layer = nn.ModuleList([RoCBertLayer(config) for _ in range(config.num_hidden_layers)])
            self.additional_attention_layer = None
        self.gradient_checkpointing = False

    def forward(
        self,
        hidden_states: torch.Tensor,
        shape_embed_in: Optional[torch.FloatTensor] = None,
        pronunciation_embed_in: Optional[torch.FloatTensor] = None,
        attention_mask: Optional[torch.FloatTensor] = None,
        head_mask: Optional[torch.FloatTensor] = None,
        encoder_hidden_states: Optional[torch.FloatTensor] = None,
        encoder_attention_mask: Optional[torch.FloatTensor] = None,
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            head_mask_index = i
            if self.additional_attention_layer is not None and i > self.additional_attention_layer:
                head_mask_index = i - 1
            layer_head_mask = head_mask[head_mask_index] if head_mask is not None else None
            past_key_value = past_key_values[head_mask_index] if past_key_values is not None else None
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if i == self.additional_attention_layer:
                    layer_outputs = torch.utils.checkpoint.checkpoint(
                        ...
                        encoder_hidden_states,
                        encoder_attention_mask,
                        shape_embed=shape_embed_in,
                        pronunciation_embed=pronunciation_embed_in,
                    )
                else:
                    layer_outputs = torch.utils.checkpoint.checkpoint(
                        create_custom_forward(layer_module),
...
class RoCBertModel(RoCBertPreTrainedModel):
    ...
    def forward(
        ...
        embedding_output, shape_embed_output, pronunciation_embed_output = self.embeddings(
            input_ids=input_ids,
        ...
        encoder_outputs = self.encoder(
            embedding_output,
            shape_embed_in=shape_embed_output,
            pronunciation_embed_in=pronunciation_embed_output,
